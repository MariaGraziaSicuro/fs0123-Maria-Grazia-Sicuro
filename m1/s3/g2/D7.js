 ...
 */
  function evenArray(n) {
    ...
    for (let i = 0; i < n.length; i++) {
      if ( n[i]%2 === 0 ){ numbers.push(i)}
    }
    return numbers
  }
  console.log(evenArray([...Array(300).keys()]));
 ...
 */
  ...
  console.log( sumArray([...Array(5).keys()]));
/* ESERCIZIO 7
...
console.log( addOneArray([...Array(5).keys()]));
/* ESERCIZIO 8
  Sostituisci ogni stringa contenuta in un array con un numero rappresentante la sua lunghezza.
  es.: ["EPICODE", "is", "great"] => [7, 2, 5]
*/
function toLength(n) {
  return n.map((a) => a.length)
}
  ...
  function giveMeRandomNoDuplicate() {
    let randNumbers = []
        for(let i = 0; i < 10; i++){
            ...
            ( !randNumbers.includes(rand) ) ? randNumbers.push(rand) : i--
        }
        return randNumbers;
}
...
*/
function findType(n) {
  return n.filter(movie => movie.Type === 'movie')
}
...
function yearMovies(n) {
  return n.filter(movie => movie.Year  >= 2000 && movie.Year  < 3000 )
}
...
function findmovie(n,imd) {
  return n.find(movie => movie.imdbID === imd )
}
...
console.log( findByWord(movies,'Endgame'));